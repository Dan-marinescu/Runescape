...
@Script.Manifest(
        name = "Planker2",
        description = "making planks in PVP",
        properties = "client = 4;"
)
public class PlankerMah extends PollingScript<ClientContext> {
    public static final int LOGS_ID = 6332;//oak logs 1521
    public static final int PLANKS_ID = 8782;// oak planks 8778
    public static final int NOTED_PLANKS_ID=8783; //noted oak planks 8779
    ...
    @Override
    public void poll() {
        if(totalLogs<28||totalLaws<20||totalCoins<100000) {
            System.out.println("out of something");
            System.out.println(totalLogs+" "+totalLaws+"  "+totalCoins);
        ...
        switch(state){
            case BANK:
                ...
                if(ctx.bank.inViewport()) {
                    ...
                    totalLaws = ctx.bank.select().id(LAWS_ID).poll().stackSize();
                    ...
                    if (ctx.inventory.select().id(LAWS_ID).poll().stackSize() < 20)
                        ctx.bank.withdraw(LAWS_ID, 100);
    ...
    private State getState() {
        ...
        if(totalPlanks>=WANTED_PLANKS&&ctx.players.select().within(10).name(TRADE_WITH).size() == 1)//got wanted amount of planks, time to trade and trader is here
            return State.TRADE;
        else if(ctx.movement.distance(atLumb)<10&&ctx.movement.distance(atLumb)>=1&&ctx.inventory.select().id(LOGS_ID).count(true) ==0)//im at cammy and got no logs in inv
            return State.BANK;
        else if(ctx.movement.distance(atLumb)<10&&ctx.movement.distance(atLumb)>=1&&ctx.inventory.select().id(LOGS_ID).count(true) >0)//im at cammy and got logs
            return State.TELEPORTHOUSE;
        else if (ctx.movement.distance(atLumb)==-1&&ctx.inventory.select().id(LOGS_ID).count(true) >0)//im at house and got logs
            return State.BUTLER;
        else if(ctx.inventory.select().id(LOGS_ID).count(true) ==0&&ctx.movement.distance(atLumb)==-1)//no more logs and at house
