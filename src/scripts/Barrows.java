...
@Script.Manifest(
        name = "Barrows",
        description = "doing barrows",
        properties = "client = 4;"
)
public class Barrows extends PollingScript<ClientContext> {
    ...
    public void poll() {
        ...
        switch(state){
            ...
            case DOOR:
                GameObject door = ctx.objects.select().id(20679).nearest().poll();
                System.out.println(door.valid());
                door.interact("Open");
    ...
    private State getState() {
        if(true)//got wanted amount of planks, time to trade and trader is here
            return State.DOOR;
    ...
    private enum State{
        BANK,TELEPORBURTHORPE,TELEPORTBANK,WALK,BUYING,DOOR
    }
